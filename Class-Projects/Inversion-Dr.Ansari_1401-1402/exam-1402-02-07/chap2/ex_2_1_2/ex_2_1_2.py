...
for indx in range(3):
    ...
    r[:, 0] = (delta/np.sqrt(lam[0])) * u[0, 0] * np.cos(theta) + (delta/np.sqrt(lam[1])) * u[0, 1] * np.sin(theta)
    #calculate the y component of the ellipsoid for all angles
    r[:, 1] = (delta/np.sqrt(lam[0])) * u[1, 0] * np.cos(theta) + (delta/np.sqrt(lam[1])) * u[1, 1] * np.sin(theta)
    #
    _x = r[:, 0] + m[m_label[0]-1]
    _y = r[:, 1] + m[m_label[1]-1]
    ax.plot(_x, _y, 'k*')
    ax.set_xlabel(f'm_{m_label[0]} ({scale_labels[m_label[0]-1]})')
    ax.set_ylabel(f'm_{m_label[1]} ({scale_labels[m_label[1]-1]})')
#    ax.set_aspect('equal')
