...
def _read_component(iput, method, dt):
    acc=[]
    for ii in range(0,6):
...
def read_v1(path_of_file,method = 'ascii', dt=None):
    global name2save
    ...
    if method == 'obspystream':
        ...
        st = Stream()
        for ii in [1,2,3]:
            tr = _read_component(File, method, dt=dt)
            st += tr
        return st
    ...
    if method == 'ascii':
        asciis = []
        for ii in [1,2,3]:
            asciis.append(_read_component(File, method, dt=dt))
        return asciis
    ...
    if method == 'both': 
        ...
        st = Stream()
        for ii in [1,2,3]:
            asci,tr = _read_component(File, method, dt=dt)
            asciis.append(asci)
            st += tr
        return asciis,st
